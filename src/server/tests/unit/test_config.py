...
class TestGitRootDetection:
    ...
    def test_find_git_root_with_git_command(self):
    ...
    def test_find_git_root_command_failure_fallback(self, mock_path_class, mock_run):
        """Test fallback when git command fails."""
        # Mock git command failure
        mock_run.side_effect = FileNotFoundError()
        
        # Create a mock path that simulates being in a subdirectory
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            subdir = temp_path / "src" / "server" / "app" / "core"
            ...
            mock_path.parent = subdir
            
            # The fallback should find the .git directory by walking up
            git_root = find_git_root()
            # Just verify it returns a Path object (exact path may vary in mocked environment)
            assert isinstance(git_root, Path)

    @patch("subprocess.run")
    def test_find_git_root_no_git_ultimate_fallback(self, mock_run):
...
class TestAppConfig:
    ...
    def test_documents_config_defaults(self):
...
class TestSettings:
    ...
    def test_settings_documents_path_computation(self):
    ...
    def test_settings_config_file_override(self):
    ...
    def test_settings_priority_env_over_config(self):
    ...
    def test_settings_git_root_absolute_path(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.dict(os.environ, env_vars):
                with patch("app.core.config.find_git_root", return_value=temp_path):
                    ...
                    assert settings.qdrant_collection == "project-vectors"
